...
class Lidar : public ISensor {
   ...
   public:
    ...
    [[nodiscard]] Eigen::MatrixXd scan(const Agents& agents, const POIs& pois, int agent_idx) const {
        ...
        std::vector<std::vector<double>> poi_values(num_sectors), agent_values(num_sectors);
        auto& agent = agents[agent_idx];  // convenient handle
        ...
        for (int i = 0; i < agents.size(); ++i) {
            ...
            auto& sensed_agent = agents[i];  //convenient handle
            ...
            agent_values[sector].push_back(1.0 / std::max(0.001, distance * distance));
        }
        ...
        for (std::size_t i = 0; i < num_sectors; ++i) {
            const std::size_t& num_agents = agent_values[i].size();
            const std::size_t& num_poi = poi_values[i].size();
            state(i) = state(num_sectors + i) = -1.0;

            if (num_agents > 0)
                state(i) = m_composition->compose(agent_values[i], 0.0, num_agents);
            if (num_poi > 0)
